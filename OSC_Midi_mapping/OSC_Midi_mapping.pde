...
void draw() {
  ...
  desenhaAtor(0); //desenha o primeiro ator (blob) rastreado
  ...
  if ((X[0] > ValorOscMin[CONTROLE_MIDI_X] && X[0] < ValorOscMax[CONTROLE_MIDI_X]) && (Y[0] > ValorOscMin[CONTROLE_MIDI_Y] && Y[0] < ValorOscMax[CONTROLE_MIDI_Y]) ) {
    if (toggle1) { 
      mandaMidi(X[0], CONTROLE_MIDI_X);
    }
    if (toggle2) { 
      mandaMidi(Y[0], CONTROLE_MIDI_Y);
    }
  }
  /*
  if (qtdeAtores >= 2) {
...
void desenhaAtor(int ator) {
  stroke(Cor[ator], Cor[ator], 255);
  fill(Cor[ator], Cor[ator], 255);
  ellipse(PosX[ator], PosY[ator], 30, 30);
}
